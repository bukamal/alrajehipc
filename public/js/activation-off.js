// js/activation.js — تفعيل مع مفتاح عام متحرك من الخادم
...
async function loadPublicKey() {
    ...
    if (!response.ok) throw new Error('فشل تحميل المفتاح العام');
...
async function verifyRsaSignature(data, signatureBase64, publicKeyPem) {
...
export async function onlineActivate(licenseCode) {
    ...
    const isValid = await verifyRsaSignature(dataToVerify, result.signature, publicKey);
